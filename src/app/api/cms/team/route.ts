...
import { createCmsTeamMember } from "@/server/cms/content-service";
import {
  cmsErrorResponse,
  cmsNoStoreJson,
  readCmsJsonObject,
...
export async function POST(request: Request) {
  ...
  try {
    const member = await createCmsTeamMember(await readCmsJsonObject(request), {
      actor: user,
      requestId: getRequestId(request),
    });
    return cmsNoStoreJson({ member }, { status: 201 });
  } catch (error) {
