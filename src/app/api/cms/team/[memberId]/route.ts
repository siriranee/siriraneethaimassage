...
export async function GET(_request: Request, context: RouteContext) {
  ...
  const member = await getCmsTeamEditorRecord(memberId);
  return member
    ? cmsNoStoreJson({ member })
    : cmsNoStoreJson({ error: "Therapist not found." }, { status: 404 });
}
...
export async function PATCH(request: Request, context: RouteContext) {
  ...
  try {
    ...
    const member = await updateCmsTeamMember(
      memberId,
      parsed.body,
      Number(parsed.body.expectedVersion),
      {
        actor: user,
        requestId,
