...
import { clearCmsSessionCookie } from "@/server/cms/auth/session";
import { cmsNoStoreJson } from "@/server/cms/http";
import { revokeManagedCmsUserSessions } from "@/server/cms/user-service";
import {
...
export async function POST(request: Request, context: RouteContext) {
  ...
  try {
    ...
    const result = await revokeManagedCmsUserSessions(userId, body, {
      actor: user,
