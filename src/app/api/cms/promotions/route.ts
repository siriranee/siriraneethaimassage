...
import { createCmsPromotion } from "@/server/cms/content-service";
...
export async function POST(request: Request) {
  ...
  try {
    const promotion = await createCmsPromotion(await readCmsJsonObject(request), {
      actor: user,
      requestId: getRequestId(request),
    });
    return cmsNoStoreJson({ promotion }, { status: 201 });
  } catch (error) {
