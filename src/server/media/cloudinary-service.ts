...
export async function cleanupExpiredCmsMediaUploads(
  value: unknown,
  actor: Pick<CmsUser, "id" | "displayName">,
  requestId?: string,
  provider: CmsCloudinaryProvider = defaultCloudinaryProvider,
  ...
  const requestedLimit =
    source.limit === undefined ? 10 : Number(source.limit);
  if (!Number.isInteger(requestedLimit) || requestedLimit < 1 || requestedLimit > 25) {
    throw new CmsMediaValidationError("Cleanup limit must be from 1 to 25.");
  }

  const config = getCloudinaryMediaConfig();
  const repository = getCmsRepository();
  const nowIso = new Date().toISOString();
  const expired = await repository.listExpiredMediaAssets(nowIso, requestedLimit);
  let removed = 0;
  let pendingFinalSweep = 0;
  let skipped = 0;
  let failed = 0;

  for (const asset of expired) {
    try {
      const result = await deleteRegisteredCmsMediaAsset(
        asset.publicId,
        actor,
        config,
        requestId,
      ...
      removed += 1;
      if ("pendingFinalSweep" in result && result.pendingFinalSweep) {
        pendingFinalSweep += 1;
      }
    } catch (error) {
      if (error instanceof CmsMediaStateError) skipped += 1;
      else failed += 1;
    }
  }
  ...
  return {
    ...
    skipped,
    failed,
    remainingMayExist: expired.length === requestedLimit,
  } as const;
}
